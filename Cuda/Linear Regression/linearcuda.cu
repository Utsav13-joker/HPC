...
int no_of_data = 1000;
__device__ int d_no_of_data =1000;
...
double longterm_error(double p, double q, double m, double c) {
  double l = (m * p) + c - q;
  return l * l;
}
__device__ double d_longterm_error(double p, double q, double m, double c) {
  double l = (m * p) + c - q;
  return l * l;
}
double rms_error(double m, double c) {
  ...
  for(x=0; x<no_of_data; x++) {
    error_sum += longterm_error(data[x].p, data[x].q, m, c);  
  }
  
  average = error_sum / no_of_data;
  
  return sqrt(average);
}
__global__ void d_rms_error(double *m, double *c,double *error_sum_arr,point_t *d_data) {
  int x = threadIdx.x + blockIdx.x *blockDim.x;
error_sum_arr[x] = d_longterm_error(d_data[x].p, d_data[x].q, *m, *c);
}
...
double error_sum_result;
double error_sum_average;

d_rms_error <<<100,10>>>(&d_dm[x],&d_dc[x],d_error_sum_arr,d_data);
...
error_sum_result+= h_error_sum_arr[j];
}
error_sum_average = error_sum_result / no_of_data;
l[x] =sqrt(error_sum_average);

if(l[x] < best_error){
best_error = l[x];
error_sum_result +=h_error_sum_arr[x];
}
error_sum_average = error_sum_result /no_of_data;
l[x] =  sqrt(error_sum_average); 
